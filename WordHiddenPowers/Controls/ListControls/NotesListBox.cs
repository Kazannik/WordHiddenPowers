...
{
	public class NotesListBox : ListControl<NotesListItem, NotesListItemNote>
	{
		...
		public event EventHandler<ItemMouseEventArgs<NotesListItem, BottomBarNote>> ItemApplyClick;
		public event EventHandler<ItemMouseEventArgs<NotesListItem, BottomBarNote>> ItemCancelClick;
		...
		protected override void OnItemMouseClick(ItemMouseEventArgs<NotesListItem, NotesListItemNote> e)
		{
			if (e.SubItem != null && e.SubItem is BottomBarNote note)
			{
				if (!note.IsAppled && note.ApplyButton.Contains(e.Location))
				{
					ItemApplyClick?.Invoke(this, 
						new ItemMouseEventArgs<NotesListItem, BottomBarNote>(e.Item, note, e.Argument, 
		...
		{
			Items.Add(new NotesListItem(note));
		}
		...
		{
			return (from NotesListItem item in Items
					where item.Note.DataRow.Equals(dataRow)
					select item.Note).First();
		}

		private NotesListItem GetNoteListItem(DataRow dataRow)
		{
			return (from NotesListItem item in Items
					where item.Note.DataRow.Equals(dataRow)
					select item).First();
	...
	public class NotesListItem : ListItem
	{
		public NotesListItem() : base() 
		{			
			Note = default;
		}

		public NotesListItem(Note note) : base(
			new NotesListItemNote[] { 
				new TitleNote(note.Category.Id, note.Subcategory.Id, note.Category.Text), 
	...
	public abstract class NotesListItemNote : ListItemNote
	{
		...
		public NotesListItemNote(string text)
		{
			this.text = text;
		}
	...
	public class TitleNote : NotesListItemNote
	{
		private Version code;
		private Size codeSize;
		private Size textSize;

		public TitleNote(int major, int minor, string text) : base(text: text) 
		{
			code = Version.Create(major: major, minor: minor);	
			codeSize = Size.Empty;
			textSize = Size.Empty;
		}
	...
	public class TextNote : NotesListItemNote
	{
	...
	public class DescriptionNote : NotesListItemNote
	{
	...
	public class BottomBarNote : NotesListItemNote
	{
