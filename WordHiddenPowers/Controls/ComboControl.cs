...
namespace WordHiddenPowers.Controls
{
	...
	public abstract class ComboControl<T> : ComboBox where T : ComboControl<T>.IComboBoxItem
	{
		...
		protected override void OnDrawItem(DrawItemEventArgs e)
		{
			...
			_ = new Size(rectText.Width - SystemInformation.VerticalScrollBarWidth - 8, rectText.Height);
		...
		public int Add(T item)
		{
			foreach (T i in Items)
			{
				if (i.Code == item.Code)
				{
					throw new ArgumentException(string.Format("Элемент с кодом {0} ранее добавлен в коллекцию.", item.Code), "item.Code");
				}
			}
		...
		{
			OnSelectedItemChanged(new ItemEventArgs(SelectedContent));
