...
{
	partial class DocumentDataSet
	{
		partial class DecimalTableDataTable
		{
			public Data.Table GetDecimalTable()
			{
				if (Rows.Count > 0)
				{
					return Data.Table.Create(Rows[0]["Value"] as string);
				}
				return Data.Table.Create("");
			}
		}
		...
		private static readonly Func<TextNotesRow, string, bool> whereGuidTextPowers = new Func<TextNotesRow, string, bool>((row, guid) => row.subcategory_guid == guid);
		private static readonly Func<TextNotesRow, string, bool> whereNotGuidTextPowers = new Func<TextNotesRow, string, bool>((row, guid) => row.subcategory_guid != guid);

		private static readonly Func<DecimalNotesRow, string, bool> whereGuidDecimalPowers = new Func<DecimalNotesRow, string, bool>((row, guid) => row.subcategory_guid == guid);
		private static readonly Func<DecimalNotesRow, string, bool> whereNotGuidDecimalPowers = new Func<DecimalNotesRow, string, bool>((row, guid) => row.subcategory_guid != guid);

		public bool IsTables => DocumentKeys.Any();

		public bool IsTableSchema => RowsHeaders.Any() && ColumnsHeaders.Any();

		public bool IsModel => IsTableSchema ||
		...
		{
			get => Setting.GetValue(Const.Globals.ML_MODEL_VARIABLE_NAME);
			set => Setting.SetValue(Const.Globals.ML_MODEL_VARIABLE_NAME, value);
		}
		...
		public void AddNote(Note note, string fileName, string caption, string description, DateTime date)
		{
			...
			CategoriesRow categoriesRow = Categories.GetRow(note.Category.Guid) as CategoriesRow;
			SubcategoriesRow subcategoriesRow = Subcategories.GetRow(note.Subcategory.Guid) as SubcategoriesRow;
			WordFilesRow wordFilesRow = WordFiles.GetRow(fileId);

			if (note.IsText)
			{
				if (!TextNotes.Exists(note: note, fileId: fileId))
					TextNotes.AddTextNotesRow(categoriesRow, subcategoriesRow, note.Description, note.Value.ToString(), note.Rating, note.WordSelectionStart, note.WordSelectionEnd, wordFilesRow, false, note.Vector);
			}
			else
			{
				if (!DecimalNotes.Exists(note: note, fileId: fileId))
					DecimalNotes.AddDecimalNotesRow(categoriesRow, subcategoriesRow, note.Description, (double)note.Value, note.Rating, note.WordSelectionStart, note.WordSelectionEnd, wordFilesRow, false, note.Vector);
			}
		}

		public IEnumerable<string> GetFilesCaption(string subcategoryGuid = "", bool not = false)
		{
			return (from row in TextNotes
				...
				.Union(from row in DecimalNotes
					   where row.RowState != DataRowState.Deleted
					   && (string.IsNullOrEmpty(subcategoryGuid) || (not
		...
		public IEnumerable<Note> GetTextNotes(bool viewHide = true)
		{
			return (from row in TextNotes
					where row.RowState != DataRowState.Deleted
		...
		{
			return (from row in DecimalNotes
					where row.RowState != DataRowState.Deleted
					&& row.subcategory_guid == subcategoryGuid
					&& !row.Hide
					select Note.Create(
		...
		/// <summary>
		///  Количество записей в подкатегории.
		/// </summary>
		/// <param name="subcategoryGuid"></param>
		/// <returns></returns>
		public int GetNotesCount(string subcategoryGuid)
		{
			return TextNotes.Count(row => row.RowState != DataRowState.Deleted
			&& row.subcategory_guid == subcategoryGuid)
				+ DecimalNotes.Count(row => row.RowState != DataRowState.Deleted
				&& row.subcategory_guid == subcategoryGuid);
		}

		public int GetGroupByFileCaptionNotesCount(string subcategoryGuid)
		{
			return TextNotes.Where(row => row.RowState != DataRowState.Deleted
			&& row.subcategory_guid == subcategoryGuid).GroupBy(x => x.WordFilesRow.Caption).Count()
				+ DecimalNotes.Where(row => row.RowState != DataRowState.Deleted
			&& row.subcategory_guid == subcategoryGuid).GroupBy(x => x.WordFilesRow.Caption).Count();
		}


		/// <summary>
		///  Сумма значений записей в подкатегории.
		/// </summary>
		/// <param name="subcategoryGuid"></param>
		/// <returns></returns>
		public double GetNotesSum(string subcategoryGuid)
		{
			return DecimalNotes
		...
		{
			IEnumerable<string> content = (from row in TextNotes
										   where row.RowState != DataRowState.Deleted
										   select Note.Create(
											   dataRow: row,
											   subcategory: GetSubcategoryOrDefault(row.subcategory_guid)))
		...
		{
			Regex regex = new("(^\\w\\s)|(\\s\\w\\s)|(\\s\\w$)|(^\\w$)|(\\W{1,})|(\\s{2,})|(\\d{1,})", RegexOptions.Multiline);
			return (from row in TextNotes
					where row.RowState != DataRowState.Deleted
					select Note.Create(
						dataRow: row,
		...
		partial class TextNotesDataTable
		{
			...
			{
				if (Get(id: id) is TextNotesRow row)
				{
					row.BeginEdit();
			...
			{
				return (from TextNotesRow row in Rows
						where row.RowState != DataRowState.Deleted
		...
		partial class DecimalNotesDataTable
		{
			...
			{
				if (Get(id: id) is DecimalNotesRow row)
				{
					row.BeginEdit();
			...
			{
				return (from DecimalNotesRow row in Rows
						where row.RowState != DataRowState.Deleted
						&& row.category_guid == category_guid
						&& row.subcategory_guid == subcategory_guid
						&& row.Description == Description
		...
		{
			if (Categories.Exists(category.Guid))
			{
		...
		{
			DataRow row = Subcategories.Rows.Add(subcategory.ToObjectsArray());
		...
		{
			SubcategoriesRow row = Subcategories.GetRow(guid) as SubcategoriesRow;
		...
		{
			DataRow row = Subcategories.GetRow(guid: subcategoryGuid);
			return Subcategory.Create(category, row);
		}
		...
		{
			if (Subcategories.Exists(subcategory.Guid))
			{
				SubcategoriesRow row = Subcategories.GetRow(guid: subcategory.Guid) as SubcategoriesRow;
		...
		partial class WordFilesDataTable
		{
			...
			public WordFilesRow GetRow(int id) => getRow(id) as WordFilesRow;
		...
		partial class DocumentKeysDataTable
		{
			...
			public DocumentKeysRow GetRow(int id) => getRow(id) as DocumentKeysRow;

			private DataRow getRow(int id) => (from DataRow row in Rows
											   where row.RowState != DataRowState.Deleted && row["id"].Equals(id)
											   select row).First();

			private DataRow getRow(string caption) => (from DataRow row in Rows
													   where row.RowState != DataRowState.Deleted && row["Caption"].Equals(caption)
													   select row).First();

			public bool Exists(int id) => (from DataRow row in Rows
										   where row.RowState != DataRowState.Deleted && row["id"].Equals(id)
										   select row).Any();

			public bool Exists(string caption) => (from DataRow row in Rows
												   where row.RowState != DataRowState.Deleted && row["Caption"].Equals(caption)
												   select row).Any();
		}
