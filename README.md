...
- Необходима отдельная утилита для предварительной обработки каталогов и файлов, которая включает в себя:
	- Переименование отдельного файла PDF в соответствии с наименованием каталога;
	- Копирование отдельных PDF файлов в единый заранее определенный каталог;
	- Автоматизация конвертирования PDF файла в документ MS Wodr (*.docx);
	- Копирование документов MS Word в отдельный каталог.
- Интерфейс взаимодействия с LLM должен быть доработан: Сейчас используется замена текста, но необходимо добавить встевку текста перед анализируемым абзацем, после него, между двумя абзацами.  	